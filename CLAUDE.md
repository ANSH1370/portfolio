...
**Demo pages for prospects/clients:** `app/demo/[slug]/page.tsx` renders a client's own assistant (tenant config fetched server-side from the RAG backend's `GET /demo/<slug>`; 404 if unknown). It reuses [components/ChatPanel.tsx](components/ChatPanel.tsx) — the chat surface extracted from the floating widget — inside [components/DemoShell.tsx](components/DemoShell.tsx) (header, disclaimer, lead form → `/api/contact`). No Navbar, no floating widget, `noindex`, and `/demo/` is disallowed in robots. `app/api/demo/[slug]/chat/route.ts` proxies to the backend's `/demo/<slug>/chat` with no FAQ/Anthropic fallback (a demo must never answer with Ansh's data). Tenants are created in the `ansh-ai-assistant` repo (see its DEMOS.md).

**Static long-form page:** "Production QLoRA Pipeline", the fine-tuning write-up at `/fine-tuning-field-notes` is a single self-contained HTML file in `public/fine-tuning-field-notes/index.html` — its own CSS, inline SVG diagrams and charts, nothing shared with the Next app. Next serves `public/` by exact path only, so `next.config.mjs` rewrites the clean URL onto `/fine-tuning-field-notes/index.html`. The page is authored in a separate folder (`../FINE_TUNING_FIELD_NOTES_SITE`, `python3 src/build.py`); to update it, rebuild there and copy `index.html` over. It is listed in `projects` in `lib/data.ts` with `internal: true` (same-tab link) and a `badge`, and in `app/sitemap.ts`.

**API routes** (all in `app/api/`, each designed to degrade gracefully rather than fail):

- `chat/route.ts` — powers the ChatWidget. Three modes, best-available-first, each falling back to the next on failure so the widget never breaks: (1) with `RAG_API_URL` set it proxies to the external `ansh-ai-assistant` RAG service (separate repo, FastAPI on Render) and returns `{reply, citations}`; (2) with `ANTHROPIC_API_KEY` it calls the Anthropic API (`ANTHROPIC_MODEL` overrides the model); (3) otherwise keyword matching against the `faq` list in `lib/data.ts`. `GET /api/chat` is a warmup ping the widget fires on page load to wake the Render free-tier instance; the RAG call waits up to 50 s (`maxDuration = 60`) and the widget shows a "waking up" hint after 6 s. History is capped at 12 messages / 1000 chars per message (the RAG backend's validation limits) and 512 output tokens in Anthropic mode.
